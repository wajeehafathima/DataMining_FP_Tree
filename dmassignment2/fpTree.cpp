...
using namespace std;
float minconfidence=0.4;          //minimumconfidence
ofstream ft,fo;
...
struct Frequent_Itemsets
{
    vector<int> v;		// vector of items in itemset.
    int cnt;			//total count of the item_set
};

vector<struct Frequent_Itemsets> arr[10];	//arr[i] has frequent_itemsets of size i.

int thresh_count =150;		
...
{
	printf("encode\n");
	std::ifstream file("input.txt");	// file is file pointer.
	string line;
	int j,i;
	for(j=0;j<768;j++)
	{
...
void Data_processing()		//Data_processing
{
...
void traverse_count(node *root,int item_count[])
{
    map<int,struct node*>::iterator it;
    for(it=root->m.begin();it!=root->m.end();it++)
    {
        ...
        traverse_count(root->m[it->first],item_count);
    }
    return;
}
void make_sets(node *root,list<pii> l)
{
    ...
    traverse_count(t1.troot,item_count);
...
void Print_tree(node * rot,vector<int> tj)
{
    map<int,struct node*>::iterator it;
    int j=0;
    node *temp=rot;
    for(it=temp->m.begin();it!=temp->m.end();it++)
    {
        ...
        ft<<"):";
        ft<<"value:"<<it->second->value<<" count:"<<it->second->counter<<" \n";
        Print_tree(it->second,tj);
        tj.pop_back();
    }
}
void make_rule_i(vector <int> l1,vector<int> r1,int right,int i,int k)
{
    ...
    if(right>=k-1)
    {
        ...
        if(float(arr[k][i].cnt)/count>minconfidence)
        {
            fo<<"{";
            for(j=0;j<l1.size();j++)
            {
...
int main()
{
    ...
    ft.open("tree.txt");
	int tot_items = 45;
	Data_processing();	//call to Data_processing.
